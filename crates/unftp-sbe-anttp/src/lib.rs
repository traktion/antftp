...
//!     let server = libunftp::Server::with_anttp(ftp_home)
...
impl Filesystem {
    /// Create a new AntTP archive backend, with the given root. No operations can take place outside
    /// of the root. For example, when the `Filesystem` root is set to `123abc`, and a client
    /// asks for `hello.txt`, the server will send it `123abc/hello.txt`.
